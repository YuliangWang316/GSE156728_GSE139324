...
remove(A,B,B_new,CHOL_CD4.data,CHOL_CD4.data_new,CHOL_CD8.data,CHOL_CD8.data_new,D,D_new,G,G_new,C,E,H,i)
metadata<-read.table("D:/GSE156728/GSE156728_metadata.txt",sep = "\t",header = TRUE,row.names = 1)
G<-as.data.frame(colnames(I))
colnames(G)<-"A"
library(tidyr)
...
M<-intersect(rownames(HNSCC.data),rownames(I))
HNSCC.data_new<-HNSCC.data[M,]
N<-cbind(HNSCC.data_new,I)
remove(HNSCC.data,HNSCC.data_new,I,M)
...
write.table(scaldata,"Treg_1C_Treg_IFNG_scaledata_mean.txt",sep = "\t")
